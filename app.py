...
# ─────────────────────────────────────────────
#  KEEP-ALIVE — evita o spin-down do Render free
# ─────────────────────────────────────────────
def _keep_alive():
    """Faz ping no próprio /healthz a cada 14 minutos para evitar sleep."""
    # Aguarda 60s para o servidor subir antes do primeiro ping
    time.sleep(60)
    app_url = os.environ.get("RENDER_EXTERNAL_URL", "")
    if not app_url:
        app.logger.info("KEEP-ALIVE: RENDER_EXTERNAL_URL não definida, desativado.")
        return
    ping_url = app_url.rstrip("/") + "/healthz"
    app.logger.info(f"KEEP-ALIVE: iniciado → {ping_url} (a cada 14 min)")
    while True:
        try:
            r = http_requests.get(ping_url, timeout=10)
            app.logger.info(f"KEEP-ALIVE: ping {r.status_code}")
        except Exception as e:
            app.logger.warning(f"KEEP-ALIVE: falhou — {e}")
        time.sleep(14 * 60)  # 14 minutos

# Inicia a thread de keep-alive em background (daemon=True para não bloquear shutdown)
